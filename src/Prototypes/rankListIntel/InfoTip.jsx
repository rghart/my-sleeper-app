...
// THROWAWAY PROTOTYPE — the tap-for-meaning affordance.
//
// Built on the app's own Popover rather than a CSS `title` or a hover tooltip,
...
//      `overflow-y-auto` body - an absolutely positioned tooltip would be
//      clipped at the sheet edge (the exact bug Popover's own comment records).
//   3. It already stops Escape from bubbling to the Sheet, so one keystroke
//      closes the tip and not the whole sheet underneath it.

// A single term, for tips anchored to one figure.
export const TermTip = ({ termKey, label }) => {
...
// The whole key, for the one "what am I looking at?" entry point.
const InfoTip = ({ children = 'What do these mean?' }) => {
    const ref = useRef(null);
    const [open, setOpen] = useState(false);
    return (
        <>
